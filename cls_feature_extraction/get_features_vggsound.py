...
for f in tqdm(path.glob("**/*.mp4")):
    ...
    counter+=1
    if counter%3000==0:
        with open('/mnt/vggsound_features_averaged', 'wb') as handle:
            pickle.dump(output_list_average, handle, protocol=pickle.HIGHEST_PROTOCOL)

        with open('/mnt/vggsound_features_no_averaged', 'wb') as handle:
            pickle.dump(output_list_no_average, handle, protocol=pickle.HIGHEST_PROTOCOL)

    mp4_version = AudioSegment.from_file(str(f), "mp4")
    mp4_version.export("/mnt/vggsound_dummy.wav", format="wav")

    try:
        audio = torch.from_numpy(vggish_input.wavfile_to_examples('/mnt/vggsound_dummy.wav'))
